# StarbridgeQuestion_Answers

> Extracted from `StarbridgeQuestion_Answers.rtf`
...
Q3: We have the Staff entity that uses multiple SharedEntityIdentiers:
    [SharedSourceSystemIdentifiers(
        ...
        typeof(Json.Json))]
Running the Poweschool Staff integration (Used only to allow FK checks for the SectionTeacher migrator to only insert if the staff exists.) caused 34 "Referred for manuakl deduplication:".
...
Jon suggested in a test env, to drop the Powerschool entry from the list of sharedSourceSystemIdentifiers. This worked and there were no manual deduplications picked up. I the added it back and it stayed working.
