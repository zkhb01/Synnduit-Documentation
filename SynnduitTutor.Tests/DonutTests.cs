...
public class DonutTests
{
    ...
    private const string Course = TestSupport.CourseId;
    ...
    private static (MasteryService mastery, DonutService donuts, CurriculumStore store) Build(TestSupport.TempDbFactory db)
    {
        ...
        return (new MasteryService(db, TestSupport.Catalog(store), opts), new DonutService(db, opts), store);
    }
    ...
    public async Task First_attempt_perfect_critical_concept_earns_all_bonuses()
    {
        ...
        var outcome = await mastery.RecordAttemptAsync(learner.Id, Course,critical, Result(1.0), new[] { "L0.E2.q1" });

        // base 1 + first-attempt 1 + perfect 1 + critical 1 = 4
        Assert.Equal(4, outcome.DonutsEarned);
        var balance = await donuts.GetBalanceAsync(learner.Id, Course);
    ...
    public async Task Late_nonperfect_noncritical_mastery_earns_base_only()
    {
        ...
        await mastery.RecordAttemptAsync(learner.Id, Course,c, Result(0.0), new[] { "L1.1.q1" }); // fail (first)
        var outcome = await mastery.RecordAttemptAsync(learner.Id, Course,c, Result(0.8), new[] { "L1.1.q2" }); // pass on retry, 80%
    ...
    public async Task Re_mastering_awards_no_additional_donuts()
    {
        ...
        var first = await mastery.RecordAttemptAsync(learner.Id, Course,c, Result(1.0), new[] { "L1.1.q1" });
        var again = await mastery.RecordAttemptAsync(learner.Id, Course,c, Result(1.0), new[] { "L1.1.q2" });
    ...
    public async Task Clearing_a_level_grants_the_completion_bonus_once()
    {
        ...
        foreach (var c in l0.Concepts)
            last = await mastery.RecordAttemptAsync(learner.Id, Course,c, Result(1.0), new[] { c.Id + ".q1" });
    ...
    public async Task Redeem_below_threshold_throws()
    {
        ...
        await mastery.RecordAttemptAsync(learner.Id, Course,store.Graph.FindConcept("L1.1")!, Result(0.8), new[] { "x" });
        var balance = await donuts.GetBalanceAsync(learner.Id, Course);
        Assert.True(balance.Available < donuts.Threshold);

        await Assert.ThrowsAsync<InvalidOperationException>(() => donuts.RedeemAsync(learner.Id, Course, "test"));
    }
    ...
    public async Task Redeem_at_threshold_issues_voucher_and_zeroes_available()
    {
        ...
        foreach (var id in new[] { "L1.1", "L1.2", "L1.3", "L1.4" })
            await mastery.RecordAttemptAsync(learner.Id, Course,store.Graph.FindConcept(id)!, Result(1.0), new[] { id + ".q1" });

        var before = await donuts.GetBalanceAsync(learner.Id, Course);
        Assert.True(before.Available >= donuts.Threshold);

        var voucher = await donuts.RedeemAsync(learner.Id, Course, "Foundations");
        ...
        var after = await donuts.GetBalanceAsync(learner.Id, Course);
