...
public class MasteryServiceTests
{
    ...
    private static MasteryService NewService(TestSupport.TempDbFactory db, CurriculumStore store) =>
        new(db, TestSupport.Catalog(store), TestSupport.DonutOpts());
    ...
    public async Task Passing_attempt_marks_concept_mastered()
    {
        ...
        var outcome = await svc.RecordAttemptAsync(learner.Id, Course,l11, Pass(), new[] { "L1.1.q1" });
    ...
    public async Task L0_placement_first_attempt_pass_is_skipped()
    {
        ...
        var outcome = await svc.RecordAttemptAsync(learner.Id, Course,l0g1, Pass(), new[] { "L0.G1.q1" });
    ...
    public async Task Repeated_failures_escalate_to_mentor()
    {
        ...
        await svc.RecordAttemptAsync(learner.Id, Course,c, Fail(), new[] { "L1.3.q1" });
        var second = await svc.RecordAttemptAsync(learner.Id, Course,c, Fail(), new[] { "L1.3.q2" });
        Assert.False(second.Mastery.EscalatedToMentor);
        Assert.Equal(1, second.Mastery.RemediationCycles);

        var third = await svc.RecordAttemptAsync(learner.Id, Course,c, Fail(), new[] { "L1.3.q3" });
    ...
    public async Task Best_score_is_retained_across_attempts()
    {
        ...
        await svc.RecordAttemptAsync(learner.Id, Course,c, Score(0.6), new[] { "L1.2.q1" });
        var outcome = await svc.RecordAttemptAsync(learner.Id, Course,c, Score(0.4), new[] { "L1.2.q2" });
